...
def listar_pedidos(request):
    try:
        ...
        # Agregar estados y zonas para el template
        estados = [
            ('pendiente', 'Pendiente'),
            ('entregado', 'Entregado'),
            ('cancelado', 'Cancelado'),
        ...
        zonas = []
        for zona_doc in db.collection('zonas').stream():
...
def importar_y_previsualizar_pedidos(request):
    ...
    with open(settings.BASE_DIR / "data/provincias.json", "r", encoding="utf-8") as f:
...
def obtener_detalle_pedido(request, pedido_id):
    try:
        ...
        response = {
            ...
            'telefono': pedido_data.get('telefono', '-'),
            'fecha': pedido_data.get('fecha', '-'),
            'estado': pedido_data.get('estado', '-'),
            'zona': pedido_data.get('zona', '-'),
            'reparto': pedido_data.get('nro_reparto', '-'),
            'peso_total': float(pedido_data.get('peso_total', 0)),
            'id': pedido_data.get('id', ''),
            'articulos': pedido_data.get('articulos', []),
        }
        # Badge color para el estado
        estado = str(pedido_data.get('estado', '')).lower()
        colores = {
            'pendiente': 'warning',
            'asignado': 'primary',
            'entregado': 'success',
        ...
        response['estado_color'] = colores.get(estado, 'secondary')
        # Historial si existe
        if 'historial' in pedido_data:
            response['historial'] = pedido_data['historial']
        else:
            response['historial'] = []
